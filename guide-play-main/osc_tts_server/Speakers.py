...
class Track():
    ...
    def __init__(self, signal, sr=None):
        ...
        # print('start__'+str(type(signal))+'')
        # passed in filename
        if type(signal) is str:
            self.sr, self.signal, self.duration = read(signal)
...
class BasicSpeaker:
    ...
    def __init__(self, point, track, id, is_ambisonic=False, signal=None, sr=0):
    ...
    def translate(self, point):
        """
        Translate a speaker to a new point.
        """
        self.point = point
        self.sr, self.sound = self.localize()
...
class AmbisonicSpeakers():
    ...
    def localize(self):
        ...
        if self.use_HTRF:
            LF_speaker = HRTFSpeaker(
                np.array([-size[0], size[1]]), Track(LF, sr=sr))
            LB_speaker = HRTFSpeaker(
                np.array([-size[0], -size[1]]), Track(LB, sr=sr))
            RF_speaker = HRTFSpeaker(
                np.array([size[0], size[1]]), Track(RF, sr=sr))
            RB_speaker = HRTFSpeaker(
                np.array([size[0], -size[1]]), Track(RB, sr=sr))
        else:
            LF_speaker = BasicSpeaker(
                np.array([-size[0], size[1]]), Track(LF, sr=sr))
            LB_speaker = BasicSpeaker(
                np.array([-size[0], -size[1]]), Track(LB, sr=sr))
            RF_speaker = BasicSpeaker(
                np.array([size[0], size[1]]), Track(RF, sr=sr))
            RB_speaker = BasicSpeaker(
                np.array([size[0], -size[1]]), Track(RB, sr=sr))
...
class HRTFSpeaker(BasicSpeaker):
    ...
    def localize(self, elev=0, plot_ir=False):
        ...
        # pick correct file
        fname = f"{str(data.path_prefix)}/hrtf/elev{nround(elev, base=10)}/H0e{str(deg).zfill(3)}a.wav"
        ...
        if plot_ir:
            ...
            plt.title(
                f"HRIR Function at {deg} degrees and {nround(elev, base=10)} elevation")
            plt.legend()
            plt.show()
