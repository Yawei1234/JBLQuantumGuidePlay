...
class Soundstage():
    ...
    def getAudio(self, id):
        ...
        for s in self.speakers:
            # print("SPEAKER", s, id)
            if s is not None and s.id == id:
                return s.get_audio()
        return None
    
    def play(self, op="overlay", save_name=None, callback=None, volume=0.5, simpleaudio=False):
        ...
        if save_name is not None:
            ...
            # play(song)
        if simpleaudio:
            playback = _play_with_simpleaudio(mixed)
            return playback
        else:
            play(mixed, volume=volume)
