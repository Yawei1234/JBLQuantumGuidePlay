    ...
    : i(
        ((t =
          "undefined" != typeof globalThis
            ? globalThis
            : t || self).JoystickController = {})
      );
})(this, function (t) {
  "use strict";
  const i = "__joysticks_identifiers__",
    e = "__active_joysticks__",
    s = { ALL: -1, LEFT: 0, MIDDLE: 1, RIGHT: 2 };
  (t.MOUSE_CLICK_BUTTONS = s),
    (t.default = class {
      constructor(t, e) {
        (this.JOYSTICK_TRANSITION = "border-radius 0.2s ease-in-out"),
          ...
          (this.addEventListeners = () => {
            const t = this.options.mouseClickButton;
            if (
              ((("string" == typeof t && Object.keys(s).includes(t)) ||
                ("number" == typeof t && 6 > t && t > -2)) &&
                (this.mouseButton = "string" == typeof t ? s[t] : t),
              this.options.dynamicPosition)
            ) {
              const t = this.options.dynamicPositionTarget || document;
              t.addEventListener("mousedown", this.dynamicPositioningMouse),
                document.addEventListener(
                  "mouseup",
                  this.removeDynamicPositioning
                ),
                t.addEventListener("touchstart", this.dynamicPositioningTouch),
                ...
                this.options.hideContextMenu &&
                  ...
                  this.joystick.addEventListener("contextmenu", (t) =>
                    t.preventDefault()
                  ));
            } else
          ...
          (this.dynamicPositioningMouse = (t) => {
            if (null !== this.identifier) return;
            if (-1 !== this.mouseButton && t.button !== this.mouseButton)
              return;
            const e = t.x + "-" + t.y;
            if (window[i].has(e)) return;
            (this.identifier = e), window[i].add(this.identifier);
            const s = t.clientY;
            (this.container.style.left = t.clientX + "px"),
              (this.container.style.top = s + "px"),
              (this.container.style.bottom = "unset"),
              (this.container.style.right = "unset"),
              document.body.appendChild(this.container),
              this.recenterJoystick(),
              this.onStartEvent(t, !1);
          }),
          (this.dynamicPositioningTouch = (t) => {
            if (null !== this.identifier) return;
            const e = t.changedTouches[0].identifier;
            if (window[i].has(e)) return;
            let s;
            (this.identifier = e), window[i].add(e);
            for (let i = 0; t.touches.length > i; i++) {
              const e = t.touches.item(i);
            ...
            if (!s) return;
            const n = s.clientY;
            (this.container.style.left = s.clientX + "px"),
          ...
          (this.removeDynamicPositioning = (t) => {
            if (null !== this.identifier) {
              if ("touchend" === t.type) {
                if (this.identifier !== t.changedTouches[0].identifier) return;
              } else if (
                -1 !== this.mouseButton &&
                t.button !== this.mouseButton
              )
                return;
              window[i].delete(this.identifier),
                (this.identifier = null),
                this.onStopEvent(t),
                this.container.remove();
            }
          }),
          (this.updateCoordinates = (t, i) => {
            (this.distance = Math.sqrt(
              Math.pow(t - this.centerX, 2) + Math.pow(i - this.centerY, 2)
            ).toFixed(4)),
            ...
            const e =
              this.distance > this.options.maxRange
                ...
                : this.distance;
            (this.x = Math.round(Math.cos(this.angle) * e)),
              (this.y = -Math.round(Math.sin(this.angle) * e)),
          ...
          (this.distortJoystick = () => {
            this.distance > 0.7 * this.options.maxRange
          ...
          (this.onStartEvent = (t, i = !0) => {
            if ("mousedown" === t.type) {
              ...
              this.identifier = t.changedTouches[0].identifier;
            }
            (this.started = !0),
          ...
          (this.onTouchEvent = (t) => {
            let i;
            for (let e = 0; t.touches.length > e; e++) {
              const s = t.touches.item(e);
              if (this.identifier === s.identifier) {
                i = s;
                break;
              }
            }
            if (!i) return;
            t.preventDefault();
            this.updateCoordinates(i.clientX, i.clientY);
          }),
          (this.onMouseEvent = (t) => {
            if (!this.started) return;
            t.preventDefault();
            this.updateCoordinates(t.clientX, t.clientY);
          }),
          (this.recenterJoystick = () => {
            (this.centerX =
              this.controller.getBoundingClientRect().left +
              this.options.radius),
              (this.centerY =
                this.controller.getBoundingClientRect().top +
                this.options.radius);
          }),
      ...
      validate() {
        window[e] &&
          ...
          window[e].some(
            (t) =>
              t.options.dynamicPosition &&
              (t.options.dynamicPositionTarget || document) ===
      ...
      init() {
        window[i] || (window[i] = new Set()),
          (this.id =
            Math.random().toString(36).substring(2, 15) +
            Math.random().toString(36).substring(2, 15)),
          (window[e] = Object.assign(window[e] || [], [this])),
          (this.style = document.createElement("style")),
          this.style.setAttribute("id", "style-" + this.id),
          (this.style.innerHTML = `\n        .joystick-container-${
            this.id
          } {\n            box-sizing: border-box;\n            position: fixed;\n            outline: none;\n            ${
            this.options.leftToRight ? "left" : "right"
          }: ${this.options.x};\n            ${
            this.options.bottomToUp ? "bottom" : "top"
          }: ${this.options.y};\n            transform: translate(${
            this.options.dynamicPosition || this.options.leftToRight
              ? "-50%"
              : "50%"
          }, ${
            this.options.dynamicPosition || !this.options.bottomToUp
              ? "-50%"
              : "50%"
          });\n        }\n    \n        .joystick-controller-${
            this.id
            ...
            2 * this.options.joystickRadius
          }px;\n            height: ${
            2 * this.options.joystickRadius
          }px;\n            z-index: 1;\n            border-radius: 50%;\n            left: 50%;\n            bottom: 50%;\n            transform: translate(-50%, 50%);\n            background: radial-gradient(#000c, #3e3f46aa);\n            transition: ${
            this.JOYSTICK_TRANSITION
          };\n        }\n        `),
      ...
      destroy() {
        if (this.options.dynamicPosition) {
          const t = this.options.dynamicPositionTarget || document;
          t.removeEventListener("mousedown", this.dynamicPositioningMouse),
            document.removeEventListener(
              "mouseup",
              this.removeDynamicPositioning
            ),
            t.removeEventListener("touchstart", this.dynamicPositioningTouch),
        ...
        window.removeEventListener("resize", this.recenterJoystick),
          ...
          Array.isArray(window[e]) &&
            (window[e] = window[e].filter((t) => t !== this));
      }
    }),
