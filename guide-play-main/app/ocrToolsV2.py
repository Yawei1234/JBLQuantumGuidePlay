...
class functions:
    ...
    def find_tables(image):
        ...
        vertically_opened = cv2.morphologyEx(
            img_bin, cv2.MORPH_OPEN, vertical_kernel)
        ...
        approx_polys = [cv2.approxPolyDP(c, e, True)
                        for c, e in zip(contours, epsilons)]
    ...
    def extract_cell_images_from_table(image):
        ...
        horizontally_dilated = cv2.dilate(
            horizontally_opened, cv2.getStructuringElement(cv2.MORPH_RECT, (1038, 1)))
        vertically_dilated = cv2.dilate(
            vertically_opened, cv2.getStructuringElement(cv2.MORPH_RECT, (1, 60)))
    ...
    def crop_to_text(image):
        ...
        horizontal_kernel = cv2.getStructuringElement(
            cv2.MORPH_RECT, (int(img_w * 0.5), 1))
        vertical_kernel = cv2.getStructuringElement(
            cv2.MORPH_RECT, (1, int(img_h * 0.7)))
        ...
        char_sized_bounding_rects = [
            (x, y, w, h) for x, y, w, h in bounding_rects if w * h > MIN_CHAR_AREA]
        ...
        bordered = cv2.copyMakeBorder(
            cropped, 5, 5, 5, 5, cv2.BORDER_CONSTANT, None, 255)
        return bordered
    ...
    def row_seperator(image, BLUR_KERNEL_SIZE_input):
        ...
        bounding_rects = functions.get_non_overlapping_rectangles(
            bounding_rects)
    ...
    def read_table_rows(cell_images_rows):
        ...
        for row in tqdm(cell_images_rows):
            ...
            ocr_name = functions.ocr_image(
                name, '-c tessedit_char_whitelist=abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789/ --psm 7', 'eng')
            ...
            for c, cnt in enumerate(cells):
                ...
                ocr_cropped = functions.ocr_image(
                    cropped, '-c tessedit_char_whitelist=0123456789 --psm 7', 'eng')
